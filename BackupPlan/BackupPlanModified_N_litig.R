...
for(i in 1:nrow(sorted_data_short)) {
  ...
  norm_file_size_data[(i: nrow(sorted_data_short)), id_col]<- sorted_data_short$N_Litigious[i]
...
saveRDS(norm_file_size_data, file = "n_litigous.rds")
# Restore the object
...
write.csv(yoy_file_size, file = "N_litigous.csv")

str(yoy_file_size)
#### saving some data
# Save an object to a file
saveRDS(yoy_file_size, file = "N_litigous.rds")
# Restore the object
...
write.csv(combined_dataframe, file = "combined_DF_files_N_litigous.csv")
saveRDS(combined_dataframe, file = "n_litigous_combined_DF.rds")
# Restore the object
#readRDS(file = "my_data.rds")

##### recovering data

combined_dataframe<- read.csv(file = "combined_DF_files_N_litigous.csv")
...
sink("summaryQ1_N_litigous2.txt")
summary(linear_model_Q1)
sink()
...
sink("summaryQ5_N_litigous2.txt")
summary(linear_model_Q5)
sink()
...
sink("summaryQ5Q1_N_litigous.txt")
summary(linear_model_Q5Q1)
sink()

linear_model_Q5Q1_2<- lm((Q5Q1_adj_return) ~ Mkt.RF + SMB + HML, data = combined_dataframe)
