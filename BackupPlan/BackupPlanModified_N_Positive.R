...
for(i in 1:nrow(sorted_data_short)) {
  ...
  norm_file_size_data[(i: nrow(sorted_data_short)), id_col]<- sorted_data_short$N_Positive[i]
...
saveRDS(norm_file_size_data, file = "n_uncertaintly_data2.rds")
# Restore the object
...
write.csv(yoy_file_size, file = "N_positive.csv")

str(yoy_file_size)
#### saving some data
# Save an object to a file
saveRDS(yoy_file_size, file = "N_positive.rds")
# Restore the object
...
write.csv(combined_dataframe, file = "combined_DF_files_N_positive.csv")

##### recovering data

combined_dataframe<- read.csv(file = "combined_DF_files_N_positive.csv")
...
sink("summaryQ1_N_positive2.txt")
summary(linear_model_Q1)
sink()
...
sink("summaryQ5_N_positive2.txt")
summary(linear_model_Q5)
sink()
